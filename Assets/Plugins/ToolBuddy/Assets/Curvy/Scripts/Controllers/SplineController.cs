...
namespace FluffyUnderware.Curvy.Controllers
{
    ...
    public class SplineController : CurvyController
    {
        ...
        public virtual void SwitchTo(CurvySpline destinationSpline, float destinationTf, float duration)
        {
            ...
            {
                DTLog.LogWarning($"[Curvy] Controller switch has a duration set to {duration}. Duration should be a strictly positive value");
        ...
        private void EventAwareMove(float distance)
        {
            ...
            while (!cancelMovement && currentDelta > 0 && infiniteLoopSafety-- > 0)
            {
                ...
                if (ReferenceEquals(candidateControlPoint, null) == false && Spline.IsControlPointVisible(candidateControlPoint))
                {
                    ...
                    {
                        float movementCompatibleNewPosition = movementCompatibleCurrentPosition + currentDelta * MovementDirection.ToInt();

                        movementCompatibleNewPosition = GetClampedPosition(movementCompatibleNewPosition, movementRelatedPositionMode, Clamping, m_Spline.Length);

                        HandleOnPositionReachedEvents(movementRelatedPositionMode,
                            movementCompatibleCurrentPosition,
                            movementCompatibleNewPosition,
        ...
        /// </summary>
        private void HandleOnPositionReachedEvents(CurvyPositionMode positionMode,
            float startPosition,
            float endPosition,
            ...
            ref bool cancelMovement)
        {
            ...
            for (int index = 0; index < OnPositionReachedList.Count; index++)
            {
                OnPositionReachedSettings settings = OnPositionReachedList[index];
                ...
                if (eventPosition > startPosition && eventPosition <= endPosition
                    || eventPosition >= endPosition && eventPosition < startPosition)
                {
                    if (settings.TriggeringDirections == TriggeringDirections.All
                        || settings.TriggeringDirections == TriggeringDirections.Forward && startPosition < eventPosition
                        || settings.TriggeringDirections == TriggeringDirections.Backward && startPosition > eventPosition)
                    {
                        ...
                        InvokeEventHandler(settings.Event, preAllocatedEventArgs, positionMode, out _, out _, out nullablePostEventsEndPosition);
